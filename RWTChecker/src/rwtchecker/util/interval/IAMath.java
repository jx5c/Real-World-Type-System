package rwtchecker.util.interval;
...
//    System.out.println("ENTERING sin2pi("+x+")");
    ...
    throws IAException
 {
  ...
  public static RealInterval min(RealInterval intvalOne, RealInterval intvalTwo){
		double lo = Math.min(intvalOne.lo, intvalTwo.lo);
		double hi = Math.min(intvalOne.hi, intvalTwo.hi);
		RealInterval resultIntval = new RealInterval(lo, hi);
		return resultIntval;
  }

  public static RealInterval max(RealInterval intvalOne, RealInterval intvalTwo){
		double lo = Math.max(intvalOne.lo, intvalTwo.lo);
		double hi = Math.max(intvalOne.hi, intvalTwo.hi);
		RealInterval resultIntval = new RealInterval(lo, hi);
		return resultIntval;
}
