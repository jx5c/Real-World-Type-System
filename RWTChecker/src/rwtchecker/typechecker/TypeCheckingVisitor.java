...
public class TypeCheckingVisitor extends ASTVisitor {
	...
	//all variables and real-world semantics that need to be recorded;
	Map<String, Map<String, HashSet<String>>> allInvAttToRecordMap = new HashMap<String, Map<String, HashSet<String>>>();
	//all invariants that need to be checked
	Map<String, ArrayList<String>> allInvariantsMap = new HashMap<String, ArrayList<String>>(); 
	//recording all values of attributes involved in invariants
	Map<String, Map<String, String>> allInvAttValMap = new HashMap<String, Map<String, String>>(); 
	...
	private boolean checkingUnits = false;
	...
	public TypeCheckingVisitor(RWTypeRulesManager manager, CompilationUnit compilationUnit, boolean unitsChecking) {
		...
		if(annotationFile!= null && annotationFile.exists()){
			...
			annotatedCount = fileAnnotations.getAllAnnotationsCount();
			//System.out.println("The number of annotation used in this file is "+annotatedCount);
		}else{
			annotatedCount = 0;
		}
	...
	/* uncomment this part to get performance improvement; if no annotation is found in this file, no type checking continue; 
	 * However, this is not always the right choice, some statement like this might have errors: var = l.getLat() + l2.getLat(); 
	 * getLat() has rwt, not no annotation needed  
	 * 
	 * @see org.eclipse.jdt.core.dom.ASTVisitor#visit(org.eclipse.jdt.core.dom.CompilationUnit)
	 */
	/*
	@Override 
	public boolean visit(CompilationUnit compliationUnit){
		if(annotatedCount == 0){
			return false;
		}
		return true;
...
//	}
	...
	private String handleGenericMethod(CompilationUnit targetedCompilationUnit, MethodInvocation methodInvocationNode, String methodDeclKey, String[] argument_cmtypes){
		...
		TypeCheckingVisitor methodDeclVisitor = new TypeCheckingVisitor(this.cmTypeOperationManager, targetedCompilationUnit, this.checkingUnits);
	...
	private void associateAttSetsWithExp(Expression exp, String annotatedType){
		if(annotatedType != null){
			...
			if(cmtype!=null){
				...
				if(!TypeCheckingVisitor.cmtypeHashSet.contains(annotatedType)){
					TypeCheckingVisitor.cmtypeHashSet.add(annotatedType);
				}
			}else{
